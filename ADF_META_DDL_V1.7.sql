-- =====================================================================
-- ADF 메타 컨트롤 DDL  V1.7   (스키마: "META_ADF")
-- V1.6 대비 변경:
--   ⑪ ux_mrs_running 락 범위를 매뉴얼 실행으로 한정(WHERE run_mode = 'MANUAL' 추가)
--      → 스케줄 실행은 마스터 락 미적용, 매뉴얼 FULL/INCR 중복 수행만 차단
-- V1.5 대비 변경:
--   ⑨ ux_ipr_running 락 범위를 FULL로 한정(WHERE ingest_type='FULL' 추가)
--   ⑩ ux_mrs_running 키에 trigger_nm 추가(트리거별 동시수행 허용)
-- V1.4 대비 변경:
...
CREATE UNIQUE INDEX ux_ipr_running ON ctl_ingest_pipeline_run
    ...
    WHERE (status = 'RUNNING' AND ingest_type = 'FULL');
